import { IsNotEmpty, IsNumber, Min, IsEnum, IsInt } from 'class-validator';
import {
  TipoMovimientoEnum,
  TipoReferenciaEnum,
...
export class CreateKardexDTO {
  ...
  @IsNotEmpty({ message: 'El producto ID no puede estar vacío' })
  ...
  @IsNotEmpty({ message: 'La referencia ID no puede estar vacía' })
  ...
  @IsNotEmpty({ message: 'El tipo de referencia no puede estar vacío' })
  @Transform(({ value }: { value: string }) => value?.trim().toUpperCase())
